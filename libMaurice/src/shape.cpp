...
void Shape::loadImage(const char *imagefile)
{
    ...
    findContours(m_alpha, contours, RETR_EXTERNAL, CHAIN_APPROX_TC89_L1);
    //findContours(m_alpha, contours, RETR_EXTERNAL, CHAIN_APPROX_NONE);
...
void Shape::computeCage()
{
    ...
    findContours(m_alpha, m_dilatedContour, RETR_EXTERNAL, CHAIN_APPROX_NONE);
...
void Shape::computeVisibilityMatrix(MatrixXi &V)
{
    ...
    //****this test takes the adjacency matrix , and checks for each edge if it lies outside the boundary
